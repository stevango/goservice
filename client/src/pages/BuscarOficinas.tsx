...
export default function BuscarOficinas() {
  ...
  const hasFilters = categoria || estado || tipoVeiculo || tipoServico;
  ...
  const categoriaLabel = (cat: string) => {
    const found = CATEGORIAS_OFICINA.find(c => c.value === cat);
  ...
  const categoriaColor = (cat: string) => {
    switch (cat) {
      ...
      case "concessionaria": return "bg-blue-100 text-blue-800 border-blue-200";
  ...
  return (
    <PublicLayout>
      <div className="container py-8">
        {/* Search Header */}
        <div className="mb-8">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight mb-2">Buscar Oficinas</h1>
          <p className="text-muted-foreground">Encontre oficinas credenciadas em todo o Brasil</p>
        </div>
          ...
          >
            ...
            {hasFilters && (
              <span className="w-5 h-5 rounded-full bg-primary text-white text-xs flex items-center justify-center">
                {[categoria, estado, tipoVeiculo, tipoServico].filter(Boolean).length}
              </span>
            )}
          </Button>
        </div>
        ...
        <div className="mb-4">
          <p className="text-sm text-muted-foreground">
            {isLoading ? "Buscando..." : `${data?.oficinas.length || 0} oficinas encontradas`}
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <h3 className="font-medium text-lg mb-2">Nenhuma oficina encontrada</h3>
            <p className="text-sm text-muted-foreground">Tente ajustar os filtros ou buscar por outra região.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <Link key={oficina.id} href={`/oficina/${oficina.id}`}>
                <div className="group p-5 rounded-xl border border-border/60 bg-card hover:shadow-lg hover:border-primary/20 transition-all duration-300 cursor-pointer h-full">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <Badge variant="outline" className={`text-xs flex-shrink-0 ${categoriaColor(oficina.categoria)}`}>
                      {categoriaLabel(oficina.categoria)}
                    </Badge>
                  </div>
